/*=====
<SceneAssetNameConverter.cs>
└作成者：takagi

＞内容
インスペクタでシーンアセットを設定すると自動で名前に変換するコンバータの実装
...
/// </summary>
[Serializable]
public struct CSceneAssetNameConverter
{
#if UNITY_EDITOR
	// クラス定義
	/// <summary>
	...
	/// </summary>
	[CustomPropertyDrawer(typeof(CSceneAssetNameConverter))]
	public class CSceneAssetNameConverterPropertyDrawer : PropertyDrawer
	{
		/// <summary>
		/// GUI表示関数
		...
		{
			// プロパティ取得
			var _SceneAsset = property.FindPropertyRelative("m_SceneAsset");	// シーンアセットのプロパティ
			var _SceneNameProperty = property.FindPropertyRelative("m_SceneName");	// シーン名のプロパティ
		
			// インスペクタに表示
			EditorGUI.ObjectField(position, _SceneAsset, label);	// 今回はアセットの変更だけで十分(※1行のdrawerと1:1の関連でつながっているため、複数個出そうとするなら他に仲介クラスを設ける必要あり)

			// データ取得
			SceneAsset _SA_Resource = _SceneAsset.objectReferenceValue as SceneAsset;	// シーンアセット本体
